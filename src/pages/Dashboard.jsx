      ...
      <main className="flex-1 p-10 overflow-y-auto">
        <header className="mb-10 flex justify-between items-end">
          ...
          <div className="text-right">
            <p className="text-[10px] text-blue-400 font-bold tracking-[0.2em] mb-1">
              NODE STATUS
            </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <StatCard
            ...
            detail="Currently online in Nexus"
          />
          <StatCard
            title="System Growth"
            value={`${stats?.growth || "0"}%`}
            detail="Performance increase"
          />
          <StatCard
            title="Total Revenue"
            value={
              stats?.revenue ? `$${(stats.revenue / 1000).toFixed(1)}k` : "$0"
            }
            detail="Total value processed"
          />
        </div>
...
const NavItem = ({ icon, label, active }) => (
  <div
    className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all ${active ? "bg-blue-600 text-white shadow-[0_0_20px_rgba(59,130,246,0.3)]" : "text-slate-400 hover:bg-white/5 hover:text-white"}`}
  >
