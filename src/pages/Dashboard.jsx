...
const Dashboard = () => {
  ...
  if (loading) return (
    <div className="h-screen w-full bg-[#F8F9FB] flex items-center justify-center">
      <div className="text-[#006D35] font-black text-xl animate-pulse tracking-widest">INITIALIZING NEXUS...</div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB] text-[#1A1C1E] flex p-5 gap-5 font-sans">
      ...
      {/* 1. Sidebar */}
      <aside className="w-64 bg-white rounded-[2.5rem] p-8 flex flex-col
      justify-between shadow-sm border border-slate-100 hidden lg:flex">
        <div>
          <div className="flex items-center gap-3 mb-12 px-2">
            <div className="w-9 h-9 bg-[#006D35] rounded-full flex items-center
            justify-center text-white font-black text-xl">D</div>
        ...
        <button onClick={logout} className="flex items-center gap-3 px-4 py-2
      ...
      <div className="flex-1 flex flex-col gap-6">
        ...
        <header className="flex justify-between items-center">
          <div className="relative w-full max-w-md">
            ...
            <input type="text" placeholder="Search task" className="w-full bg-white rounded-full py-3.5 pl-12 pr-4
            shadow-sm border-none text-sm outline-none focus:ring-1 focus:ring-green-100" />
          </div>
          <div className="flex items-center gap-5">
            <div className="p-2.5 bg-white rounded-full shadow-sm text-slate-400 cursor-pointer hover:bg-slate-50 transition-colors"><Bell size={20} /></div>
            <div className="flex items-center gap-3 bg-white p-1.5 pr-5 rounded-full shadow-sm cursor-pointer border border-slate-50">
              ...
              <div>
                <p className="text-xs font-black leading-none">Toteck Michael</p>
                <p className="text-[10px] text-slate-400 mt-1">tmichael20@mail.com</p>
              </div>
            </div>
          </div>
        </header>
            ...
            <div className="flex-[2.2] space-y-6">
              <div className="flex justify-between items-end">
                <div>
                  ...
                  <p className="text-slate-400 text-sm mt-1">Plan, prioritize, and accomplish your tasks with ease.</p>
                </div>
                <div className="flex gap-3">
                  <button className="bg-[#006D35] text-white px-6 py-3 rounded-full
                   text-sm font-black flex items-center gap-2 shadow-lg">
                    <Plus size={18} strokeWidth={3} /> Add Project</button>
                </div>
              </div>

              {/* Stats Grid */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <StatBox title="Total Projects" value="24" growth={data?.overview?.growth} active />
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                    ...
                    <BarChart data={data?.analytics}>
                      <Bar dataKey="views" radius={[6, 6, 6, 6]}>
                        {data?.analytics?.map((entry, index) => (
                          <Cell key={index} fill={index === 2 ? '#006D35' : '#D1FAE5'} />
                        ))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                ...
                <div className="bg-white p-7 rounded-[2.5rem] shadow-sm">
                  ...
                  <div className="bg-[#F8F9FB] p-5 rounded-3xl">
                    <p className="text-sm font-black">Meeting with Arc Company</p>
                    <button className="mt-5 w-full bg-[#006D35] text-white py-3
                    rounded-2xl text-xs font-black">Start Meeting</button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 space-y-6 min-w-[300px]">
              <div className="bg-white p-8 rounded-[2.5rem] shadow-sm">
                ...
                <div className="relative w-40 h-40 flex items-center justify-center mx-auto">
                  <span className="absolute text-3xl font-black">{data?.overview?.growth}%</span>
                  <svg className="w-full h-full transform -rotate-90">
                    <circle cx="80" cy="80" r="70" stroke="#F1F5F9" strokeWidth="14" fill="transparent" />
                    <circle cx="80" cy="80" r="70" stroke="#006D35" strokeWidth="14" strokeDasharray={440}
                    strokeDashoffset={440 - (440 * (data?.overview?.growth || 0)) / 100}
...
const StatBox = ({ title, value, growth, active, isSmall }) => (
  <motion.div whileHover={{ y: -4 }} className={`p-6 rounded-[2rem] shadow-sm border transition-all ${active ? 'bg-[#006D35] text-white border-transparent' : 'bg-white text-slate-800 border-slate-50'}`}>
  ...
  return (
    ...
    className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100">
      <div className="flex justify-between items-center mb-10">
        <div>
          <h2 className="text-2xl font-black tracking-tight text-slate-900">Team Members</h2>
          <p className="text-sm text-slate-400 mt-1">Manage your team and their access levels.</p>
        </div>
        <button className="bg-[#006D35] text-white px-6 py-3 rounded-full text-sm font-black flex items-center gap-2">+ Invite Member</button>
      </div>
          ...
          <tbody className="divide-y divide-slate-50">
            {users?.map((user) => (
              <tr key={user.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="py-5">
                  <span className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase ${user.status === 'active' ? 'bg-green-50 text-green-600' : 'bg-slate-50 text-slate-400'}`}>
                    {user.status}
                  </span>
                </td>
                <td className="py-5 text-sm text-slate-500">{new Date(user.joinDate).toLocaleDateString()}</td>
                <td className="py-5 text-right pr-4"><MoreHorizontal className="inline text-slate-300" /></td>
              </tr>
            ))}
          </tbody>
        </table>
